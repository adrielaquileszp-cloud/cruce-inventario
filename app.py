...
def obtener_recepciones_por_folios(models, uid, folios_s):
    """Trae TODAS las recepciones vinculadas a los folios S dados."""
    if not folios_s:
        return {}

    # Buscar órdenes de compra vinculadas a los folios S
    # El nombre de la PO tiene formato "P44342 (S74001)" - el folio S está en el name
    folios_list = list(folios_s)

    # Buscar en lotes de 50 para no sobrecargar
    purchase_ids = []
    for i in range(0, len(folios_list), 50):
        lote = folios_list[i:i+50]
        # Buscar POs cuyo nombre contenga alguno de los folios S
        domain = [('company_id', '=', COMPANY_TIENDAS_ID)]
        or_conditions = [('name', 'ilike', f) for f in lote]
        # Construir dominio OR
        if len(or_conditions) > 1:
            or_domain = ['|'] * (len(or_conditions) - 1) + or_conditions
        else:
            or_domain = or_conditions
        pos = models.execute_kw(ODOO_DB, uid, ODOO_PASS,
    ...
    if not purchase_ids:
        # Fallback: buscar recepciones directamente en el rango amplio
        return {}

    # Buscar pickings vinculados a esas órdenes de compra
    pickings = query(models, uid, 'stock.picking', [
