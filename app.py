...
# ============================================================
def obtener_surtidos(models, uid, fecha_inicio, fecha_fin, partner_ids):
    # PASO 1: Descubrir qué folios S tuvieron actividad en el rango de fechas
    pickings_rango = query(models, uid, 'stock.picking', [
    ...
    # Extraer folios S únicos
    folios_encontrados = set()
    for p in pickings_rango:
        folio = p['origin'] or ''
        if folio.startswith('S'):
            folios_encontrados.add(folio)

    if not folios_encontrados:
        return {}

    # PASO 2: Traer TODAS las entregas de esos folios (sin filtro de fecha)
    # Esto asegura que las entregas parciales anteriores o posteriores se incluyan
    pickings = query(models, uid, 'stock.picking', [
        ('company_id', '=', COMPANY_CEDIS_ID),
        ('picking_type_code', '=', 'outgoing'),
        ('state', '=', 'done'),
        ('origin', 'in', list(folios_encontrados)),
        ('partner_id', 'in', partner_ids),
...
def obtener_recepciones(models, uid, fecha_inicio, fecha_fin, folios_s_encontrados):
    # Buscar recepciones que estén vinculadas a los folios S encontrados
    # Primero buscamos en el rango de fechas
    pickings_rango = query(models, uid, 'stock.picking', [
    ...
    # También buscamos recepciones fuera del rango que correspondan a los folios S
    # Para esto buscamos por purchase_id que contenga los folios S
    pickings_extra = []
    if folios_s_encontrados:
        # Buscar todas las recepciones completadas que no están en el rango
        all_pickings = query(models, uid, 'stock.picking', [
            ('company_id', '=', COMPANY_TIENDAS_ID),
            ('picking_type_code', '=', 'incoming'),
            ('state', '=', 'done'),
        ], ['id', 'name', 'partner_id', 'origin', 'date_done', 'purchase_id'],
        {'limit': 5000, 'order': 'date_done desc'})

        ids_rango = {p['id'] for p in pickings_rango}
        for p in all_pickings:
            if p['id'] not in ids_rango:
                purchase_name = p['purchase_id'][1] if p['purchase_id'] else ''
                folio_s = extraer_folio_s(purchase_name)
                if folio_s and folio_s in folios_s_encontrados:
                    pickings_extra.append(p)

    pickings = pickings_rango + pickings_extra
...
if ejecutar:
    if not sucursales_sel:
        ...
        if not uid:
            ...
            progress = st.progress(0, text="Consultando surtidos del CEDIS...")
            surtidos = obtener_surtidos(models, uid, str(fecha_inicio), str(fecha_fin), sucursales_sel)
            progress.progress(40, text=f"Surtidos encontrados: {len(surtidos)} folios")

            progress.progress(50, text="Consultando recepciones en tiendas...")
            folios_s = set(surtidos.keys())
            recepciones = obtener_recepciones(models, uid, str(fecha_inicio), str(fecha_fin), folios_s)
            progress.progress(80, text=f"Recepciones vinculadas: {len(recepciones)} folios")
